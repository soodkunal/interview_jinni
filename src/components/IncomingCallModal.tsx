...
type CallRequest = {
  id: number
  requester_id: number
  // We need to join the 'users' table to get the name
  users: {
...
export default function IncomingCallModal() {
  ...
  // 1. Fetch the user's own profile on load
  useEffect(() => {
  ...
  // 2. When the profile is loaded, subscribe to the real-time channel
  useEffect(() => {
  ...
  const requesterName = incomingCall.users?.full_name || incomingCall.users?.email || 'Someone'
